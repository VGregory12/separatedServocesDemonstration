...
import separatedServices.domain.*;
...
public class BenefitsCertificateController {
    ...
    public ResponseEntity<Object> outMessageBenefitsCertificate(
        ...
        ResponseEntity<Object> result = getResult(
                dataIn.getData(),
                req,
                wr.toString());
        log.info("---END-------");
    ...
    private ResponseEntity<Object> getResult(
            BenefitsCertificateDataIn dataIn,
            HttpServletRequest req,
            String jsonParams
    ) throws GISSZException {

        final String serviceName = "Справка о размере пособия на детей и периоде его выплаты";
        Connection session = null;
        Utils util = new Utils();
        List data = new ArrayList<>();
        String method = "BenefitsCertificatePackage";
        ...
        try {
            ...
            while (rs1.next()) {
                ...
                while (rs2.next()) {
                    ...
                    if (benefit2.getText().equals("Пособие семьям на детей в возрасте от 3 до 18 лет в период воспитания ребенка в возрасте до 3 лет")) {
                        tempAmount.getItems().add(benefit2);
                    } else {
                        montlyBenefits.getItems().add(benefit2);
                    }
                    row1_count++;
                }
                ...
                while (rs3.next()) {
                    ...
                    onlyBenefits.getItems().add(benefit3);
                    row2_count++;
                }
            ...
            montlyBenefits.getItems().addAll(tempAmount.getItems());
